...
func (m *EnvManager) toEnvInfo(managed *ManagedEnv) EnvInfo {
	...
	orderedServices := managed.Config.OrderedServiceNames()
	for _, name := range orderedServices {
		service := managed.Services[name]
		serviceConfig := managed.Config.Services[string(name)]
		port, _ := m.portRegistry.GetPhysicalPort(managed.BasePort, indexOfService(orderedServices, name))
...
func (m *EnvManager) unregisterEnvRoutes(managed *ManagedEnv) {
	for _, name := range managed.Config.OrderedServiceNames() {
		m.proxy.Unregister(serviceRouteHost(name, managed.EnvID))
	}
}
