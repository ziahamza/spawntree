...
describe('parseLogLine', () => {
  it('parses JSON SSE data from server', () => {
    const raw = '{"ts":"2026-04-01T10:42:31Z","service":"api","stream":"stdout","line":"GET /health 200 2ms"}'
  ...
  it('detects stderr as error', () => {
    const raw = '{"ts":"2026-04-01T10:42:31Z","service":"worker","stream":"stderr","line":"connection refused"}'
    const line = parseLogLine(raw)
    expect(line.isError).toBe(true)
  })

  it('detects error keywords in message', () => {
    const raw = '{"ts":"","service":"web","stream":"stdout","line":"Error: ECONNREFUSED 127.0.0.1:5432"}'
    const line = parseLogLine(raw)
    expect(line.isError).toBe(true)
