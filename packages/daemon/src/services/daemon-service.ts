...
export class DaemonService extends ServiceMap.Service<
  DaemonService,
  {
    ...
    getEnv(
      ...
      envId: string,
      repoPath?: string,
    ): Effect.Effect<GetEnvResponse, DaemonError>;
    ...
    ): Effect.Effect<ConfigPreviewResponse, DaemonError>;
      ...
      Effect.gen(function* () {
        ...
        const shutdown = Effect.gen(function* () {
          for (const session of previewSessions.values()) {
            yield* Effect.tryPromise({
              try: () => envManager.deleteEnv(session.repoId, session.envId),
                ...
                new InternalError({ code: "PREVIEW_SHUTDOWN_FAILED", message: toMessage(error) }),
          ...
          for (const env of envs) {
            yield* Effect.tryPromise({
              try: () => envManager.downEnv(env.repoId, env.envId),
                ...
                new InternalError({ code: "ENV_SHUTDOWN_FAILED", message: toMessage(error) }),
            }).pipe(Effect.catchTag("InternalError", () => Effect.void));
          }
          ...
          yield* Effect.tryPromise({
            try: () => proxyManager.stop(),
              ...
              new InternalError({ code: "PROXY_STOP_FAILED", message: toMessage(error) }),
          }).pipe(Effect.catchTag("InternalError", () => Effect.void));
        ...
        const daemonInfo = Effect.gen(function* () {
          const [row] = yield* Effect.tryPromise({
            try: () =>
              catalog.select({ count: sql<number>`count(*)`.as("count") }).from(reposTable),
        ...
        const getEnv = Effect.fn("DaemonService.getEnv")(function* (
          repoRef: string,
          envId: string,
          repoPath?: string,
        ) {
          const resolved = yield* resolveRepoEnv(
            catalog,
            envManager,
            previewSessions,
            repoRef,
        ...
        const downEnv = Effect.fn("DaemonService.downEnv")(function* (
          repoRef: string,
          envId: string,
          repoPath?: string,
        ) {
          ...
          logDaemonMessage("down env", {
            repoId: resolved.repoId,
            envId: resolved.env.envId,
            repoPath: resolved.env.repoPath,
          });
          yield* Effect.tryPromise({
        ...
        const deleteEnv = Effect.fn("DaemonService.deleteEnv")(function* (
          repoRef: string,
          envId: string,
          repoPath?: string,
        ) {
        ...
        const registerRepo = Effect.fn("DaemonService.registerRepo")(function* (
        ...
        const stopInfra = Effect.fn("DaemonService.stopInfra")(function* (
          request: StopInfraRequest,
        ) {
          yield* Effect.tryPromise({
            try: async () => {
        ...
        const restoreDb = Effect.fn("DaemonService.restoreDb")(function* (
        ...
        const getWebRepoTree = Effect.fn("DaemonService.getWebRepoTree")(function* (
        ...
        const probeAddPath = Effect.fn("DaemonService.probeAddPath")(function* (path: string) {
          return yield* Effect.try({
            try: () => probePath(path),
              ...
              new BadRequestError({ code: "INVALID_PATH", message: toMessage(error) }),
          });
        });

        const addFolder = Effect.fn("DaemonService.addFolder")(function* (
          request: AddFolderRequest,
        ) {
          logDaemonMessage("add folder", {
            path: request.path,
            remoteName: request.remoteName,
            scanChildren: request.scanChildren,
          });
          ...
          if (probe.isGitRepo && !request.remoteName) {
            const remotes = yield* Effect.try({
              try: () => detectRemotes(probe.path),
                ...
                new InternalError({ code: "REMOTE_DETECTION_FAILED", message: toMessage(error) }),
            });
            if (remotes.length > 1) {
              return { remotes };
            }
          ...
          if (probe.isGitRepo) {
            const imported = yield* importGitRepoPath(
              catalog,
              probe.path,
              request.remoteName,
              true,
            ...
            publish({
              type: "repo.updated",
              repoId: imported.repo.id,
              repoSlug: imported.repo.slug,
            });
            return {
              repo: imported.repo,
        ...
        const relinkClone = Effect.fn("DaemonService.relinkClone")(function* (
          repoSlugValue: string,
          cloneId: string,
          request: RelinkCloneRequest,
        ) {
          ...
          yield* Effect.tryPromise({
            try: async () => {
              ...
              new BadRequestError({ code: "RELINK_FAILED", message: toMessage(error) }),
          });
          publish({ type: "repo.updated", repoSlug: repoSlugValue });
        });

        const deleteClone = Effect.fn("DaemonService.deleteClone")(function* (
          repoSlugValue: string,
          cloneId: string,
        ) {
          const repo = yield* getRepoBySlug(catalog, repoSlugValue);
        ...
        const archiveWorktree = Effect.fn("DaemonService.archiveWorktree")(function* (
          repoSlugValue: string,
          request: ArchiveWorktreeRequest,
        ) {
          const repo = yield* getRepoBySlug(catalog, repoSlugValue);
          ...
          const info = yield* Effect.try({
            try: () => inspectGitPath(request.path, repo.defaultBranch, false),
              ...
              new InternalError({ code: "WORKTREE_INSPECTION_FAILED", message: toMessage(error) }),
          });
          if (!info.canArchive) {
            return yield* new ConflictError({
          ...
          yield* Effect.tryPromise({
            try: async () => {
              ...
              new InternalError({ code: "WORKTREE_ARCHIVE_FAILED", message: toMessage(error) }),
          });
          publish({ type: "repo.updated", repoSlug: repoSlugValue });
        });
        ...
        const testConfig = Effect.fn("DaemonService.testConfig")(function* (
        ...
        const startConfigPreview = Effect.fn("DaemonService.startConfigPreview")(function* (
          request: ConfigPreviewRequest,
        ) {
          const repoPath = yield* normalizeRepoPath(request.repoPath);
          ...
          logDaemonMessage("start preview", {
            repoPath,
            previewId,
            serviceName: request.serviceName ?? "all",
          });
          ...
          }).pipe(Effect.tapError(() => Effect.sync(() => safeRemove(configPath))));
          ...
          logDaemonMessage("preview ready", {
            previewId,
            envId: env.envId,
            repoId: env.repoId,
            repoPath: env.repoPath,
        ...
        const stopConfigPreview = Effect.fn("DaemonService.stopConfigPreview")(function* (
          request: ConfigPreviewStopRequest,
        ) {
        ...
        const saveConfig = Effect.fn("DaemonService.saveConfig")(function* (
          ...
          const gitRoot = yield* Effect.try({
            try: () => validateGitRepo(repoPath),
              ...
              new BadRequestError({ code: "NOT_A_GIT_REPO", message: toMessage(error) }),
          });
              ...
              : resolve(
                  yield* Effect.try({
                    try: () => findWorktreeForBranch(gitRoot, defaultBranchName(repoPath)),
                      ...
                      new ConflictError({
                        code: "DEFAULT_BRANCH_WORKTREE_NOT_FOUND",
...
function buildGitPathInfoMap(
  ...
  for (const clone of clones) {
    try {
      gitPaths[clone.path] = inspectGitPath(
        clone.path,
        repo.defaultBranch,
        (envCounts.get(clone.path) ?? 0) > 0,
      );
    } catch {
...
async function enrichRepo(
  ...
  for (const env of envs) {
    ...
    if (
      env.services.some((service: ServiceInfo) => service.status === "failed") &&
      overallStatus !== "running"
    ) {
...
function resolveRepoEnv(
  ...
  return Effect.tryPromise({
    try: async () => {
      ...
      const [byId] = bySlug
        ? [bySlug]
        : await catalog.select().from(reposTable).where(eq(reposTable.id, repoRef)).limit(1);
      const repo = bySlug ?? byId;
      if (repo) {
        const envs = await listRepoEnvsForRepo(catalog, envManager, repo);
        const found = envs.find(
          (env) => env.envId === envId && (!repoPath || env.repoPath === repoPath),
        );
        if (found) {
...
function runConfigTest(repoPath: string, content: string): ConfigTestResponse {
  ...
  const missingHealthchecks = (
    Object.entries(validation.config.services) as Array<[string, ServiceConfig]>
  )
...
function buildConfigSuggestions(repoPath: string): ConfigSuggestResponse {
  ...
  if (existsSync(resolve(repoPath, "pnpm-lock.yaml"))) {
    ...
    existsSync(resolve(repoPath, "bun.lock")) ||
    existsSync(resolve(repoPath, "bun.lockb"))
  ) {
...
function inferPort(command: string) {
  for (const pattern of [/--port(?:=|\s+)(\d{2,5})/, /PORT=(\d{2,5})/, /-p\s*(\d{2,5})/]) {
